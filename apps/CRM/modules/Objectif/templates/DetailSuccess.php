		...
		<div class="row">
			<table class="display table table-striped table-hover" id="tableau">
        		...
        		<tr>
        			...
        			<td <?php if(isset($row["id_firme"])): ?> onClick="window.open('<?php echo url_for('ConsulterFirme',array("id" => $row["id_firme"])) ?>')" style="cursor: pointer;" <?php endif; ?>> 
        			 <?php echo $c; ?>
        			</td>
        			
        			<?php else:?>
        			
        			<td <?php if($k=="commentaire"): ?>
        					onClick="window.open('<?php echo url_for('activiteCommerciale',array("act" => "detail_commentaire", "code_firme" => $row["code_firme"])) ?>')"
