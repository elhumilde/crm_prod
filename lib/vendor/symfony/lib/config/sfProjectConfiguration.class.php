...
class sfProjectConfiguration
{
  ...
  public function setRootDir($rootDir)
  {
    ...
    $root_new = dirname($rootDir);
    sfConfig::add(array(
      'sf_root_dir' => $rootDir,

      // global directory structure
      'sf_apps_dir_new'    => $root_new,
  ...
  static public function getApplicationConfiguration($application, $environment, $debug, $rootDir = null, sfEventDispatcher $dispatcher = null)
  {
    ...
    $rootDir_new = dirname($rootDir);
    
    if (!file_exists($file = $rootDir_new.'/'.$application.'/config/'.$class.'.class.php') && !file_exists($file = $rootDir.'/apps/'.$application.'/config/'.$class.'.class.php'))
    {
