...
abstract class sfAction extends sfComponent
{
  ...
  public function preExecute()
  {
  	...
  	if(defined('TTS_BD')){
  	  	Common::$__TWF_DB = TTS_BD;
  	  	form::$__TWF_DB = TTS_BD;
  	  	filter::$__TWF_DB = TTS_BD;
  	...
  	if($this->getUser()->isAuthenticated()){
	  	if($this->isSecure()){
	  	// set secure -------------------------------------------------------------
	  	$this->secure = new TTSSecure($this);
	  	$this->secure->start();
	  	
	  	if(!$this->getUser()->hasAttribute("secure")){
	  		$this->getUser()->setAttribute("secure", $this->secure->javascriptSecure()); 
	  	}
  	...
  	//disable/enable UTF8 in Form Class ---------------------------------------
  	FORM::$UTF8 = false;
  	//-------------------------------------------------------------------------
